...
export async function generateStaticParams() {
  const rows = await db
    ...
    .where(
      and(
        eq(research.status, "published"),
        or(isNull(research.publishedAt), lte(research.publishedAt, sql`(unixepoch())`))
      )
    );
...
export default async function ResearchDetail({
  ...
  const row = await resolveForView("research", slug, preview);
  if (!row) notFound();

  const tags = parseTags(row.tags);
  const url = absoluteUrl(`/research/${row.slug}`);
  ...
  return (
    <article className="max-w-3xl mx-auto px-6 md:px-8 py-12">
      ...
      <RelatedPosts kind="research" excludeId={row.id} tags={tags} />
    </article>
  );
}
